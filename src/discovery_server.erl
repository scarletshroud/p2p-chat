...
start_link() ->
	gen_server:start_link({local, ?MODULE}, ?MODULE, #state{connected_peers = []}, []).

init(State) ->
	{ok, State}.

join_network(Peer) ->
	gen_server:call(?MODULE, Peer).
...
handle_call(Peer, _From, #state{connected_peers = ConnectedPeers}) ->
	Response = case ConnectedPeers of 
		[] -> 
			{discover, nil};
		_ -> 
	...
	end,
	{
		reply, 
		Response,
		#state{connected_peers = [Peer | ConnectedPeers]}
	}.
