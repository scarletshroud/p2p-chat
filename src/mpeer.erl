...
-export([add_peer/2, closest_peer/2, closest_peer_in_network/3, create_peers/2, join/1]).

-include("definitions.hrl").
-include("protocol.hrl").

add_peer(_, {Size, Tree}) when Size >= ?id_size -> {Size, Tree};

add_peer(Peer, Peers) ->
  gb_trees:enter(
    Peer#peer.id,
    {Peer#peer.username, Peer#peer.server_pid, Peer#peer.client_pid},
    Peers
  ).

closest_peer(SearchKey, {_, Tree}) -> closest_peer(SearchKey, Tree);
...
closest_peer_in_network(Peer, _, Id) when Peer#peer.id == Id -> Peer;

closest_peer_in_network(Peer, Peers, Id) ->
  Result = closest_peer(Id, Peers),
  case Result#peer.id == Id of
    true -> Result;
    false -> recursive_find_closest(Peer, Id, nil, Result, ?max_hops)
  end.


recursive_find_closest(_, _, _, Result, 0) -> Result;
recursive_find_closest(_, _, Result, Result, _) -> Result;
recursive_find_closest(Result, _, _, Result, _) -> Result;
...
recursive_find_closest(Peer, Id, _, Result, RemainingHops) ->
  ...
  server:find_closest(
    ...
    _ -> {error, "Unable to find the closest peer."}
  end.
...
create_peers(Peer, Peers) ->
  ...
  NewPeers =
    lists:foldl(
      fun (NetworkPeer, NewPeers) -> add_peer(NetworkPeer, NewPeers) end,
      gb_trees:empty(),
      BestNetworkPeers
    ),
  PeersList =
    lists:foldl(
      fun
        ({Id, {Username, ServerId, ClientId}}, PeersList) ->
          [
            #peer{id = Id, username = Username, server_pid = ServerId, client_pid = ClientId}
            | PeersList
          ]
      end,
      [],
          ...
          _ ->
            case server:get_peers(RemotePid, #get_peers_request{peer = Peer}) of
              #get_peers_response{peers = RemotePeers} ->
                ...
                Peer#peer.client_pid;

              _ -> io:format("Unable to get peers. Timeout.")
            end
        end;

      _ -> io:format("~p Discovery (~p) timed out~n", [self(), Peer])
    end
  catch
    _:_ ->
      Pids = read_pids(),
      ...
      server:update_peers(
        Peer#peer.server_pid,
        #update_peers_request{peers = Peers, secret_key = SecretKey}
      ),
      Peer#peer.client_pid
  end.


find_available_peer([]) -> {error, "Unable to find avaible peer"};
...
find_available_peer(Pids) ->
  ...
  try server:ping(Pid, #ping_request{sender = self()}) of
    ...
    _:_ -> find_available_peer(lists:delete(StringPid, Pids))
  end.
    ...
    {ok, ServerPid} ->
      case client:start(Id, Username, SecretKey, ServerPid, Keys) of
        {ok, ClientPid} ->
          Peers =
            add_peer(
              #peer{id = Id, username = Username, server_pid = ServerPid, client_pid = ClientPid},
              EmptyPeers
            ),
          server:update_peers(
            ServerPid,
